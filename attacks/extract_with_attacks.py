...
class AttackCorrelator:
    ...
    def is_attack_traffic(
        ...
        """
        Check if traffic matches a logged attack.
    ...
    def label_dataframe(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Add ground-truth attack labels to a dataframe.
        ...
        """
        logger.info("Correlating traffic with attack log...")
        ...
        # Check each row (this could be slow for large datasets)
        confirmed_count = 0
        
        for idx in df.index:
            is_attack, info = self.is_attack_traffic(
                df.at[idx, '@timestamp'],
                str(df.at[idx, 'src_ip']),
                str(df.at[idx, 'dest_ip'])
            )
            
            if is_attack:
                df.at[idx, 'attack_confirmed'] = True
                df.at[idx, 'attack_id'] = info['attack_id']
                df.at[idx, 'attack_category'] = info['category']
                df.at[idx, 'attack_subcategory'] = info['subcategory']
                df.at[idx, 'attack_technique'] = info['technique_id']
                df.at[idx, 'attack_tool'] = info['tool']
                confirmed_count += 1
