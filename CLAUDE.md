...
SOC-ML is a supervised machine learning pipeline for threat detection in a HomeLab SOC v2 environment. It ingests Suricata alerts and network flow records from OpenSearch, engineers behavioral features, and trains an XGBoost binary classifier to distinguish attacks from noise/informational/benign traffic. A Purple Team attack framework generates ground-truth labeled training data.
...
SOCOpenSearchClient (src/utils/opensearch.py)
    |  scrolls & caches queries
DataExtractor (src/data/extract.py)
    |  extracts alerts + flows, labels binary, stratified sampling, temporal split
    |
...
FeatureEngineer (src/data/features.py)
    |  40+ behavioral features: network, flow stats, derived ratios, one-hot categorical, IP features
    |
...
- **Behavioral features only** — severity and signature_id are excluded because Suricata sets severity=1 for attacks (leaks the label). Only network/flow/derived features are used.
...
| Module | Class | Role |
|--------|-------|------|
| `src/utils/opensearch.py` | `SOCOpenSearchClient` | OpenSearch connection, scroll search, aggregations, caching |
| `src/data/extract.py` | `DataExtractor` | Alert/flow extraction, binary labeling, balanced dataset creation |
| `src/data/extract_with_attacks.py` | `AttackCorrelator` | Correlates traffic with `attack_log.csv` for ground-truth labels |
| `src/data/features.py` | `FeatureEngineer` | Feature engineering with fit/transform pattern (40+ features) |
...
- `config/opensearch.yaml` — connection, indices (`fluentbit-default`), query settings, date ranges
- `config/features.yaml` — label definitions (noise/info/attack signature patterns), feature specs, sampling strategy
